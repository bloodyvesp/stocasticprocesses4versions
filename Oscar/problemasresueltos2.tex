...
Por lo argumentado inicialmente, $\Cp\subset\Ll$. Por lo tanto si demostramos que $\Ll$ es un $\lambda$-sistema, lo cual es BASTANTE parecido al procedimiento usado en los ejercicios anteriores (usando propiedades de la integral) y será omitido, concluiremos que efectivamente, la función $D_n$ propuesta es la derivada de Radon-Nikodym de $\p^p|_{\F_n}(A_n)$ respecto a $\p^{1/2}|_{\F_n}(A_n)$. Además, por la primera parte del inciso anterior es una martingala  Si se demuestra que $D$ es uniformemente integrable, habremos demostrado, por el inciso 1), que $\p^p\ll\p^{1/2}$. (FALTA DEMOSTRAR UI).
\end{proof}
